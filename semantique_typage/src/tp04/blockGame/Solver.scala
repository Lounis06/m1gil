...
trait Solver extends GameEnv {
  ...
  def startBlock: Block = Block(start, start)

  /** Le bloc est-il à la verticale sur la position finale ? */
  def done(b: Block): Boolean = b.p1 == goal && b.p2 == goal && b.isStanding
   ...
   *  voisin valide et de l'historique enrichi du mouvement ayant permis
   *  d'atteindre ce voisin.
   */
  def neighboursAndHistory(b: Block, history: History):
      List[(Block, History)] = b.validNeighbours.map{case (b, m) => (b, history ::: List(m))}
  ...
  def selectNewNeighbours(neighbours: List[(Block, History)],
      explored: Set[Block]): List[(Block, History)] = neighbours.filterNot{case (b, h) => explored.contains(b)}
